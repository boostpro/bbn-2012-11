...
#ifndef __GNUC__ // Workaround for http://gcc.gnu.org/bugzilla/show_bug.cgi?id=55206
...
{
 private:
    typedef boost::shared_ptr<json_base> stored_type;
 public:
    ...
    {
        stored_value = make_storage(x, boost::is_integral<T>(), boost::is_floating_point<T>());
        return *this;
    }
 ...
 private:
    ...
    template <class T>
    static stored_type make_storage(T x, boost::true_type, boost::false_type)
    {
        return boost::make_shared<json_store<json_integer> >(x);
    }

    static stored_type make_storage(bool x, boost::true_type, boost::false_type)
    {
        return boost::make_shared<json_store<bool> >(x);
    }
    
    template <class T>
    static stored_type make_storage(T x, boost::false_type, boost::true_type)
    {
        return boost::make_shared<json_store<json_float> >(x);
    }
    
    template <class T>
    static stored_type make_storage(T& x, boost::false_type, boost::false_type)
    {
        return boost::make_shared<json_store<T> >(boost::move(x));
    }
